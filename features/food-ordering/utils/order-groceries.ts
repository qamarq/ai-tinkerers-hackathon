...
export function createBrowserTools(page: Page) {
  return {
    ...
    click: tool({
      description:
        "Kliknij element. Metody: 'text' (po widocznym tekście), 'css' (selektor CSS), 'role' (po roli ARIA, np. button/link z nazwą).",
      inputSchema: z.object({
        selector: z
          .string()
    ...
    getHTML: tool({
      description:
        "Pobierz fragment HTML strony – pomaga znaleźć poprawne selektory CSS. Podaj selektor CSS elementu-rodzica.",
      inputSchema: z.object({
        selector: z
          .string()
          .default("body")
          .describe("Selektor CSS elementu (np. '[role=dialog]', 'main', 'body')"),
        maxLength: z.number().default(3000).describe("Max długość zwracanego HTML"),
      }),
    ...
    type: tool({
      ...
      inputSchema: z.object({
        ...
        selector: z.string().optional().describe("Opcjonalny selektor CSS pola"),
        clearFirst: z
          .boolean()
          .default(true)
    ...
    addToCart: tool({
      ...
      inputSchema: z.object({}),
      execute: async () => {
        const btn = page.locator('[data-test-id="ItemCardStepperIncrement"]').first();
        if (await btn.isVisible({ timeout: 1000 }).catch(() => false)) {
          await btn.click();
          return "Dodano do koszyka";
        }
        const fallback = page.locator('[aria-label="Dodaj jeszcze jedno"][role="button"]').first();
    ...
    wait: tool({
      description: "Poczekaj chwilę (max 1s). Używaj RZADKO – zazwyczaj lepiej od razu zrobić screenshot zamiast czekać.",
      inputSchema: z.object({
        ms: z.number().default(500).describe("Czas oczekiwania w ms (max 1000)"),
      }),
...
export async function orderGroceries(items: string[]): Promise<OrderResult> {
  ...
  const cookiePath = new URL("../../scripts/wolt-cookies.txt", import.meta.url).pathname;
  ...
  await page.goto("https://wolt.com/pl/pol", { waitUntil: "domcontentloaded", timeout: 60_000 });
      ...
      onStepFinish: (step) => {
        ...
        for (const tr of step.toolResults) {
          if (tr.toolName !== "screenshot") {
            console.log(`[${tr.toolName}]`, JSON.stringify(tr.output).slice(0, 150));
          } else {
